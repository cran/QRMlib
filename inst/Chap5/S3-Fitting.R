...
#following line:
library(QRMlib);
#if you have previously opened the ftse100, smi,  and FXGBP.RAW data sets AND saved 
...
#requires(fCalendar) #version 240.10068
#requires(its)
...
TS1 <- cut(ftse100, "1990-11-08", "2004-03-25"); # TS1 is ftse100 truncated.
...
#mergeSeries().  In 240.10068, mergeSeries() has been deprecated and we must use merge();
#INDEXES.RAW <- mergeSeries(TS1Augment, TS2Augment@Data);
INDEXES.RAW <- merge(TS1Augment,TS2Augment);
#clean up:
rm(TS1, TS1Augment, TS2Augment);
#call utilityFunction.R plot.timeSeriesIts()(identical to plot.timeSeries from version 221.10065 of fCalendar) 
...
#call utilityFunction.R plot.timeSeriesIts()(identical to plot.timeSeries from version 221.10065 of fCalendar):
plot.timeSeriesIts(INDEXES);
#Create a subset of the timeSeries using dates only from 01/01/1994 to 12/31/2003:
#Hence we have ftse100 and smi data only between the selected dates. Since the 'from'
#date is not included, we set it to '1993-12-31' rather than '1994-01-01'
#In version 240.10068, fCalendar uses cut() rather than cutSeries() to select a subset from timeseries:
